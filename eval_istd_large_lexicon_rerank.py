...
def build_large_lexicon(gt_data):
    ...
    for label in gt_data.values():
        w = normalize_text(label)
        if MIN_WORD_LEN <= len(w) <= MAX_WORD_LEN:
            words.add(w)

    for w in top_n_list("en", LEXICON_SIZE):
        w = normalize_text(w)
        if MIN_WORD_LEN <= len(w) <= MAX_WORD_LEN:
            words.add(w)
...
def confusion_edit_distance(a, b):
    ...
    for i in range(1, n + 1):
        for j in range(1, m + 1):
            ...
            delete = dp[i - 1][j] + 0.85
            insert = dp[i][j - 1] + 0.85
...
def predict_topk_probs(model, image_path):
    ...
    for _ in range(MAX_LEN):
        ...
        input_token = torch.tensor([best_idx], dtype=torch.long, device=DEVICE)
...
def get_candidate_pool(pred_norm, lexicon):
    ...
    for word in lexicon:
        len_diff = abs(len(word) - len(pred_norm))

        if len_diff > MAX_LEN_DIFF:
            continue

        overlap = char_overlap_score(word, pred_norm)

        if overlap < MIN_OVERLAP:
            continue

        edit = confusion_edit_distance(word, pred_norm)

        if edit > MAX_EDIT_COST:
            continue
...
def rerank(pred, step_probs, lexicon):
    ...
    pool = get_candidate_pool(pred_norm, lexicon)

    candidates = []

    for word, edit, overlap, len_diff in pool:
        visual = visual_score_candidate(word, step_probs)
        prefix = prefix_score(word, pred_norm)
        freq = freq_score(word)

        final = (
            VISUAL_WEIGHT * visual
            - EDIT_WEIGHT * edit
            - LENGTH_WEIGHT * len_diff
...
def main():
    ...
    lexicon = build_large_lexicon(gt_data)
    ...
    for filename in image_files:
        ...
        pred, step_probs = predict_topk_probs(model, image_path)
        pred_norm = normalize_text(pred)

        reranked, top_candidates = rerank(pred_norm, step_probs, lexicon)
    ...
    for filename, gt, pred, reranked, topc in wrong_cases:
        ...
        for word, final, visual, edit, overlap, prefix, freq in topc:
            print(
                f"   {word:20s} "
                f"final={final:.4f} "
                f"visual={visual:.4f} "
